...
export async function openFieldModal(button) {
  cy.get('button[class^="button_button_"]', {timeout: 40000}).contains(button).as('modalButton')
  cy.get('@modalButton', {timeout: 40000}).should('be.visible')
  cy.get('@modalButton', {timeout: 40000}).click({force: true})
...
export async function outBoundSelectDateTicket(getprice=false) {
  openFieldModal('Date')
  cy.get('#portal div[class*=\'datecell_available_\']', {timeout: 30000})
 ...
 * @return {string} The CSS color of the button
 */
...
/**
   ...
   * @param {string} buttonText - The label text of the button
   * @return {string} The CSS background color of the button
   */
...
export async function blockingWait(ms) {
